...
BYTE * BMPFile::LoadBMP(CString fileName, 
						UINT *width, 
						UINT *height,BITMAPINFOHEADER* bihdr)
{
	BITMAP inBM;
    ...
    *width=0; *height=0;
	...
	else 
	{
		...
		////////////////////////HGLOBAL chemistry//////////////////////////////////
		DWORD iSize=filesize - sizeof(BITMAPFILEHEADER);
		//HGLOBAL hMem=::GlobalAlloc(GMEM_MOVEABLE,iSize);
		LPBYTE hMem = (LPBYTE)new BYTE[iSize];
		bihdr=(LPBITMAPINFOHEADER)hMem;

		bihdr->biSize=sizeof(BITMAPINFOHEADER);
		bihdr->biWidth=inBM.bmWidth;
		bihdr->biHeight=inBM.bmHeight;
		bihdr->biPlanes=inBM.bmPlanes;
		bihdr->biBitCount=inBM.bmBitsPixel;
		bihdr->biCompression=compression;
		bihdr->biSizeImage=sizeimage;
		bihdr->biXPelsPerMeter=xscale;
		bihdr->biYPelsPerMeter=yscale;
		bihdr->biClrUsed=colors;
		bihdr->biClrImportant=impcol;

		//::GlobalUnlock(hMem);
		////////////////////////////////////////////////////////////////////////////
		// read colormap/RGBQUAD array
...
////////////////////////////////////////////////////////////////////
//  allocates and returns an RGB buffer containing the image
//  but instead of a file buffer is read from memory.
//  necessary memory pointer is retrieved from BITMAPINFOHEADER * 
////////////////////////////////////////////////////////////////////
BYTE* BMPFile::FindBits(BITMAPINFOHEADER *pbihdr)
{
	...
	BYTE* memory=(BYTE*)pbihdr;			//initialize our "good-old" memory pointer
										//instead of file pointer
    //all those filling stuff...
	w = pbihdr->biHeight;
	h = pbihdr->biWidth;

	sizeimage=pbihdr->biSizeImage;
	compression=pbihdr->biCompression;
	bpp=pbihdr->biBitCount;
	xscale=pbihdr->biXPelsPerMeter;
	yscale=pbihdr->biYPelsPerMeter;
	colors=((pbihdr->biClrUsed == 0)? (1 << pbihdr->biBitCount): (pbihdr->biClrUsed));
	impcol=pbihdr->biClrImportant;

	RGBQUAD *colormap = (RGBQUAD*)((BYTE*)pbihdr + sizeof(BITMAPINFOHEADER));
	memory += (sizeof(BITMAPINFOHEADER));
	BYTE* rawbits=((BYTE*)(colormap) + colors * sizeof(RGBQUAD));
	memory += colors * sizeof(RGBQUAD);
	...
	BYTE* outBuf=(BYTE *) new BYTE[bufsize];
	if (outBuf==NULL) 
	{
		m_errorText="Memory alloc Failed";
	}
	else 
	{
			...
			for (row = h-1; row >= 0; row--) 
			{
				...
				if (bpp == 24) 
				{
					...
					//we have read a row::pointer arithmatic
					memory += row_size;

					// read DWORD padding
					while ((memory - rawbits) & 3) 
					{
						char dummy;
						if (memcpy(&dummy,memory,1)==NULL) 
						{
							m_errorText="Read Error";
							delete [] outBuf;
							return NULL;
				...
				{	// 1, 4, or 8 bit image
					...
					for (int col=0; col < w; col++) 
					{
						...
						{
							bit_count = 8;
							if (memcpy(&inbyte,memory,1)==NULL) 
							{
								m_errorText="Read Error";
								delete [] outBuf;
								delete [] colormap;
